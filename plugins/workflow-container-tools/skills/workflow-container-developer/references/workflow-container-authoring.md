...
После успешной semantic verification именованного профиля общий runtime, не освобождая profile lease, выполняет `POST <mcp_playwright_profile_writeback_candidate_url>?profile=<physical-profile>` с пустым body. URL уже привязан платформой к текущему `WorkflowRun`, точному `DataSource` и изменяемому префиксу, из которого был материализован capability source; runtime не передает эти значения, policy или filesystem path повторно. Run-local platform control endpoint применяет сохраненную policy, просит `browser-vpn-runtime` завершить процесс и атомарно заменить `/runtime/mcp_playwright_profile/writeback_candidate/` полным snapshot выбранного профиля. Успешный no-op или публикация завершаются `204`; другой HTTP status является инфраструктурной ошибкой. Candidate всегда один: последняя успешно завершенная подходящая invocation заменяет предыдущую. SQLite, реестр кандидатов, sequence number и сравнение содержимого для этого не нужны.
...
- изолированный browser route без target, однократную инициализацию target из immutable capability source, reset из явного source перед первой action-попыткой, каждым transport retry и correction attempt, а также verification без reset;
