...
| --- | --- |
| `workflow-control-center` | Платформа выполнения: создание и публикация `WorkflowSourceVersion`, exact-source build, обязательная проверка image, schema snapshots, формы `Workflow` и `WorkflowRun`, mapping `DataSource`, scoped manifests и publications, `WorkflowRunController`, run-local control service, runtime capabilities и состояние запуска. |
| `workflow-container-contract` | Независимые от среды выполнения модели и загрузчики `workflow.yaml`, включая build, command, publisher tests, data mounts и capabilities, `versions.yaml`, ограниченный профиль `input.schema.json`, input validation, определения input migrations, `WorkflowRunStatus`, `McpPlaywrightProfileWritebackPolicy` и общий `WorkflowResult` для границы с платформой. |
| `workflow-container-runtime` | Необязательная общая реализация платформенного интерфейса: базовые классы workflow и конфигураций, адаптер стандартной среды и control protocol, жизненный цикл, run-local concurrency и browser-profile lease, стандартные пути, атомарная запись JSON, SQLite current state, запуск Codex, рендеринг общих prompt, материализация и общие валидаторы. |
...
`WorkflowResultT` не управляет `DataSource` publications или диагностическими списками платформы. Эти данные принадлежат `workflow-control-center` и не добавляются в предметный результат ради удобства платформы.

### 2.2. Направление зависимостей
Конкретный контейнер этой implementation family зависит от закрепленных версий `workflow-container-contract` и `workflow-container-runtime`. Независимый `WorkflowSource` может реализовать платформенный интерфейс без общего runtime package. Контейнер может обращаться к готовому browser runtime через переданный MCP URL, но не зависит от внутреннего устройства `browser-vpn-runtime`.
...
`WorkflowDefinition.build.dockerfile_path` задает безопасный относительный Dockerfile внутри того же source root. `WorkflowDefinition.command` является полной runtime command, а необязательный `WorkflowDefinition.test.command` является publisher-owned test command. `WorkflowDefinition.data_mount_list` объявляет стабильные mount keys, непересекающиеся container roots, access modes, допустимые типы `DataSource` и allowlist publication triggers; `/runtime` не маппится, mounts под `/input` остаются read-only, writable roots принадлежат `/workspace` или `/result`, а trigger не публикует весь mount неявно. Capability ссылается на mount только через `data_mount_key`. Поля `image`, `data_source_list` и `data_container_list` не входят в целевой контракт.
...
data_mount_list:
  - mount_key: secret
    container_path: /input/.secret
    access_mode: read_only
    data_source_type: secret
    publication_trigger_list: []
  - mount_key: workspace
...
`command` является полной runtime command; платформа не добавляет CLI arguments. Она передает только `WORKFLOW_RUN_ID`, `WORKFLOW_INPUT_PATH=/input/input.json`, `WORKFLOW_RUNTIME_PATH=/runtime`, `WORKFLOW_CONTROL_URL`, `WORKFLOW_CAPABILITY_CONFIG_PATH=/input/capability.json`, стандартные filesystem roots и versioned run-local HTTP control protocol. `/input` неизменяем, platform files внутри него не перекрываются data mounts, `/input/.secret` является условным read-only secret mount, `/runtime` принадлежит запуску и не является `DataSource`, а `/workspace` и `/result` становятся persistent только через объявленные mounts и подтвержденные publications.

`Workflow` связывает каждый mount key с принадлежащим пользователю `DataSource` и logical base path. Один или разные mounts могут использовать один или разные `DataSource`, если resolved writable scopes не пересекаются несовместимым образом. Существующий parent head не делится неявно: пользователь выполняет отдельную atomic split operation до mapping, а runtime при overlap завершается conflict. Mount типа `secret`, доступный image, остается read-only и не имеет ordinary publication triggers; trusted capability может публиковать собственное scoped state через control protocol. `WorkflowRun` фиксирует exact mount snapshots как упорядоченные наборы immutable manifests. Только mount с ordinary publication trigger получает отдельный run base path и сохраненное expected absence; read-only mount или mount без такого trigger не требует split ради запуска. Capability snapshot независимо фиксирует expected head state своей source scope, включая отсутствие head. Safepoint указывает stable step identity, transition identity и canonical publication request list; каждый entry содержит mount key и safe path относительно container mount root. Платформа выводит destination scopes из сохраненного run base path, logical execution identity и relative paths, безопасно копирует trees в immutable staging и фиксирует request/content digests до object upload. Group idempotency key выводится из run logical execution identity и transition identity без request payload, остается стабильным между replacement attempts и не включает attempt fence. Затем одна final DB-транзакция блокирует затронутые `DataSource` в deterministic id order, повторно проверяет overlap и публикует все scoped manifests, append-only publications, path heads и step completion. Первая ordinary publication ожидает сохраненное отсутствие head, первая trusted capability publication ожидает head state capability snapshot, а следующая publication той же scope в том же run сериализуется control service и ожидает manifest предыдущей accepted publication. Staging и uploads непересекающихся scopes выполняются параллельно; короткая final transaction одного `DataSource` сериализуется. Conflict одного entry откатывает всю группу. Повтор из той же или следующей replacement attempt продолжает exact intent по стабильному group idempotency key, несовпадающий replay отклоняется, а partial visibility и last-writer-wins запрещены.

Один `WorkflowRun` является durable логическим запуском, а каждый последовательный запуск source-owned command выполняется отдельной platform execution attempt и отдельным Kubernetes Job. Одновременно разрешена только одна active attempt. Per-attempt suspended identity gate может задержать старт command до durable persistence exact Job UID и current fence, но не запрещает будущий replacement Job. При recoverable failure до terminal intent платформа durable-сохраняет pending replacement, переводит прежнюю attempt в `stopping`, отзывает ее fence и запрашивает остановку. Пока terminal или absent state прежнего Job и отсутствие его active Pods не доказаны, run остается `working`, controller продолжает reconciliation без нового Job и пользовательского `Retry`. Сразу после proof controller автоматически terminal-сохраняет прежнюю attempt и запускает replacement с теми же exact image, input, logical execution identity, последним принятым safepoint и persistent `/runtime`, control staging и capability state, но новым current `WORKFLOW_CONTROL_URL`; runtime обязан восстанавливаться идемпотентно и продолжать persisted transition с прежними identities. Terminal request использует reserved terminal execution identity, open `WorkflowResult`, stable transition identity и canonical terminal publication list, а current proxy добавляет platform-owned fence. Control service сначала durable-сохраняет immutable terminal intent на еще `working` run и возвращает receipt. После receipt image завершает process без дальнейшей предметной работы, replacement запрещен, а только совместимый exit exact Job связанной с intent attempt разрешает одной final transaction принять result, все terminal publications и terminal status. Platform Job не имеет собственного automatic process retry; bounded sequential replacement attempts принадлежат platform controller. Пользовательский повтор после terminal run создает новый `WorkflowRun`. Exit без intent не принимает image-authored result и обрабатывается platform recovery policy; измененный replay, несовместимый exit после intent или истекший grace оставляет result и terminal publications непринятыми. Платформа гарантирует serialization attempts и идемпотентность принятых control transitions, но arbitrary external side effects конкретного source должен дедуплицировать сам source.
...
| `BrowserRuntimeCapability` | Run-local MCP router URL, immutable исходный snapshot профиля из текущего `DataSource` и platform control URL staging writeback candidate. |
...
| `CodexExecutionError` после исчерпания низкоуровневых попыток | Граница `CodexRunner` | Пробрасывает исключение наружу; предметный workflow не превращает его в `error_list`. Сохраненные файлы остаются recovery bundle общей реализации. Exit текущей execution attempt без terminal intent сохраняет pending replacement по platform recovery policy; после доказанной остановки controller автоматически запускает последовательный replacement Job с последнего safepoint, а non-recoverable failure или исчерпанный budget завершает `WorkflowRun`. |
...
Платформа запускает полную `command` в current execution attempt exact `WorkflowRun` без дополнительных CLI arguments. Последовательный replacement Job разрешен только после доказанной остановки и fencing прежней attempt и получает тот же persistent run state; параллельные attempts запрещены. Недоказанная остановка сохраняет run `working` и pending replacement, который controller автоматически запускает сразу после proof с последнего принятого safepoint без пользовательского `Retry`. Identity и расположение границ передаются через `WORKFLOW_RUN_ID`, `WORKFLOW_INPUT_PATH=/input/input.json`, `WORKFLOW_RUNTIME_PATH=/runtime`, `WORKFLOW_CONTROL_URL` и `WORKFLOW_CAPABILITY_CONFIG_PATH=/input/capability.json`. `WORKFLOW_CONTROL_URL` указывает на proxy текущей attempt, который владеет scoped platform credential и fencing token; image не получает отдельный control credential или fence. Регистрация, safepoint, terminal result и cancellation принадлежат versioned run-local HTTP control protocol.

`/input` неизменяем и содержит полный input и объявленные read-only inputs. Secret input при наличии монтируется только для чтения в `/input/.secret`. Наша общая реализация до запуска workflow копирует нужные изменяемые private runtime files в доступный для записи pod-local `/runtime/.secret` с корректными правами. Workflow и Codex используют только runtime copy; `CODEX_HOME` не указывает на read-only mount. Такое копирование является поведением `workflow-container-runtime`, а не обязательной внутренней реализацией custom image. Передача secret mount является явным grant exact image digest; платформа не обещает полную redaction произвольного stdout, stderr или network traffic недоверенного процесса.

`/runtime` является private run-local state и не маппится на `DataSource`. `/workspace` и `/result` являются условными container roots: persistence появляется только у объявленного `data_mount_list` entry, который `Workflow` связал с `DataSource` и logical base path. Один physical storage credential image не получает.
...
Публикацию выполняет только control service для exact mount или capability snapshot. Для safepoint он принимает stable step identity, transition identity и canonical publication request list; для terminal publication использует reserved terminal execution identity. Service сам разрешает все source subtrees и destination scopes, descriptor-relative копирует их без symbolic-link traversal в immutable control-owned staging на persistent platform-only run-local storage, проверяет staging и фиксирует intent с отдельными request/content digests. Этот storage не смонтирован в workflow image и отделен от image-visible `/runtime`. Group idempotency key выводится из logical execution identity run и transition identity без request payload, остается стабильным между replacement attempts и не включает attempt fence. Upload читает только staging. Одна final DB-транзакция блокирует затронутые `DataSource` в deterministic id order, проверяет parent/child overlap, создает scoped `DataSourceManifest` и append-only `WorkflowRunDataSourcePublication`, обновляет все `DataSourcePathHead` и завершает transition. Пустой image list завершает step без изменения persistent mounts только при отсутствии policy-required capability candidate в той же группе. Повтор transition с тем же canonical request и content из той же или следующей replacement attempt продолжает тот же intent; несовпадающий replay отклоняется. Staging и uploads непересекающихся scopes допускают параллельность; conflict одного entry откатывает всю группу и не оставляет partial visible state.

Terminal endpoint через current execution proxy сначала сохраняет immutable intent вместе с fence attempt на еще `working` run и возвращает durable receipt. После receipt workflow process прекращает предметную работу и завершается; replacement запрещен, а result, terminal publications и terminal status становятся видимыми вместе только после совместимого exit exact Kubernetes Job, связанного с intent attempt. Exit без intent не принимает image-authored result и может привести к serialized replacement по platform policy; несовпадающий replay, несовместимый exit после intent или истекший grace не публикуют image-authored terminal state частично.
...
#### Writeback candidate
`WorkflowBrowserConfigBase.mcp_playwright_profile_writeback_policy` определяет writeback без скрытых defaults. `mcp_playwright_profile_name_prefix=""` разрешает любой именованный профиль; непустое значение допускает только физические имена, начинающиеся с этого case-sensitive префикса. Изолированные профили никогда не участвуют. Пустой `workflow_run_status_list` полностью отключает candidate и writeback. `created` не является допустимым условием; остальные значения принадлежат общему `WorkflowRunStatus`. Policy проверяет accepted terminal transition платформенного `WorkflowRun`, а не `VerificationResult.status` или предметный `WorkflowResult.status`: принятый `WorkflowResult(status="success")` выбирает `done`, принятый `WorkflowResult(status="failed")` выбирает domain `failed`, а cooperative cancellation может выбрать `cancelled`. Infrastructure `failed` или platform-owned `cancelled` без accepted terminal intent не создают terminal publication group и не выполняют writeback.

После успешной semantic verification именованного профиля общий runtime, не освобождая profile lease, выполняет `POST <mcp_playwright_profile_writeback_candidate_url>?profile=<physical-profile>` с versioned JSON body, содержащим stable step identity и transition identity, и явным runtime-owned конечным положительным тайм-аутом. URL уже привязан платформой к текущему `WorkflowRun`, exact capability snapshot, одному `DataSource` и logical scope; runtime не передает эти значения, policy или filesystem path повторно. Run-local control endpoint сериализует candidate transitions этой scope и проверяет сохраненную policy. Пустой список статусов или профиль, отфильтрованный по имени, дает успешный no-op без обращения к browser router.

Для подходящего профиля control endpoint вызывает внутренний browser-router endpoint `POST /runtime/mcp-playwright-profile/writeback-candidate?profile=<physical-profile>` с пустым body. Browser router завершает backend выбранного профиля, атомарно заменяет `/runtime/mcp_playwright_profile/writeback_candidate/` его полным snapshot и возвращает `204`. Этот `/runtime` является capability-private mount root browser router и не смонтирован в workflow image. Control service descriptor-relative копирует candidate в immutable platform-only staging, детерминированно привязанный к capability scope, step identity и transition identity, и вычисляет content digest. Повтор тех же identities использует существующее staging, а другой content digest отклоняется. После успешного staging control service атомарно переводит run-local current pointer на этот candidate. Current candidate всегда один: последняя полностью staged подходящая invocation текущего запуска заменяет предыдущую. Отдельный SQLite или DB registry кандидатов не создается.

Candidate endpoint не создает отдельную `DataSource` publication и возвращает внешний `204` после immutable staging и current-pointer update. Если policy содержит `working`, следующий общий safepoint тех же step/transition identities обязан добавить platform-required capability publication из keyed staging в ту же atomic group, что image-requested publications и step completion. Первая capability publication ожидает head state capability snapshot, включая отсутствие head, а следующая publication той же scope в том же run ожидает manifest предыдущей accepted publication. Если policy содержит только terminal statuses, step safepoint не публикует candidate, а terminal protocol добавляет current candidate в общую terminal group при соответствующем завершении запуска. Внутренние сервисы не выполняют `commit`, `rollback` и не открывают вложенную транзакцию. Другой HTTP status на любом участке является инфраструктурной ошибкой.

DBOS step считается успешно завершенным только после необходимого candidate update и принятия общего safepoint. `TimeoutError` и другие transport failures остаются инфраструктурными исключениями, освобождают profile lease при выходе из текущего вызова и не превращаются в correction feedback. При recovery уже принятого `result.json` общий runtime повторно получает тот же profile lease, подтверждает candidate с теми же step/transition identities и повторяет safepoint. Для `working` replay возвращает существующую atomic publication group либо продолжает ее immutable staging; для terminal-only policy replay подтверждает тот же run-local candidate transition без persistent publication. Измененный content после зафиксированного candidate или safepoint intent отклоняется, а unreferenced immutable uploads удаляются по retention policy.

Непустой `workflow_run_status_list` разрешает обновление run-local candidate после каждой успешно проверенной подходящей invocation независимо от перечисленных терминальных статусов. Значение `working` дополнительно означает safepoint: platform добавляет keyed candidate в atomic step group до возврата из DBOS step. Терминальные `done`, `failed` и `cancelled` означают одну publication current candidate в общей accepted terminal group при соответствующем переходе статуса запуска; terminal state без accepted intent не публикует candidate. Если policy содержит и `working`, и терминальный статус, обе publications выполняются как явно запрошенные переходы с разными idempotency keys.

Отсутствие candidate допустимо при отключенной policy, отсутствующей browser capability, использовании только изолированных профилей, отсутствии invocation с подходящим именованным профилем или до первого успешного подходящего invocation при `working`. Факт использования именованного профиля определяется существованием его каталога только под `/runtime/mcp_playwright_profile/profile/`; отдельный usage registry не создается. Принятая image-authored terminal transition завершается ошибкой только тогда, когда ее терминальный статус требует writeback, существует хотя бы один каталог физического профиля, подходящий под policy, но ни одна подходящая invocation не завершилась успешно и candidate не существует. Infrastructure failure или platform cancellation без accepted terminal intent не выполняют эту проверку и не создают terminal publication group. Параллельные непересекающиеся scopes публикуются независимо; пересекающаяся publication с устаревшим expected head завершается conflict и не заменяет concurrent result.
...
- полный runtime `command` без добавленных CLI arguments, стандартные interface environment и filesystem roots, не более одной active execution attempt, persisted pending replacement, сохранение run `working` без ручного `Retry` при недоказанной остановке, немедленный автоматический replacement с последнего safepoint после proof, сохранение persistent state между attempts, rejection stale fence, регистрацию в control service, safepoint, terminal intent/current-Job-exit acceptance без replacement и cancellation;
- `data_mount_list`, exact run snapshots, scoped manifests, immutable staging и intent recovery, atomic multi-mount safepoints, empty-list step completion, payload-independent group idempotency, deterministic `DataSource` locking, compare-and-swap всех path heads, rollback всей группы при одном conflict и параллельные непересекающиеся uploads.
...
class McpPlaywrightProfileRuntime:
    ...
    def writeback_candidate_stage(self, route: McpPlaywrightProfileRoute) -> None:
        ...
