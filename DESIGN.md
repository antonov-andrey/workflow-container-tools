...
Все новые GitHub repositories этой системы создаются у personal owner `antonov-andrey`. Marketplace repository `agent-plugins` размещается как `antonov-andrey/agent-plugins` посредством rename существующего `antonov-andrey/workflow-container-tools`, а новый standards provider размещается как `antonov-andrey/project-standards`.
...
Все live local directories, symlinks и configured paths, которые представляют checkout, worktree, marketplace source, установленный plugin или cache переименованной identity, должны соответствовать её новому canonical имени. Repository checkout, worktree и marketplace-source copies используют repository name; installable-plugin source и cache copies используют plugin identifier. Для утверждённого перехода `workflow-container-tools` это означает repository и marketplace-source path `agent-plugins`, а также installable-plugin path `workflow-container-agent-tools`.
